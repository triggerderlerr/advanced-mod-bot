...
// Event: VoiceStateUpdate
  ...
  // ================== BUTTON İNTERACTİON'S ==========================
    rowreply: (client, oldState, newState) => {
      const interaction = newState
 ...
 //   interaction.showModal(modal)
      ...
      client.on('interactionCreate', async (interaction) => {
        ...
        if (interaction.customId === 'rowlimit') {
...
// ================== BUTTON İNTERACTİON'S ==========================
rowreply2: (client, oldState, newState) => {
    const interaction = newState;

    const modal = new ModalBuilder()
        .setCustomId('myModal') // Kanal adı için modal
        .setTitle('Kanal Adı');

    const modal2 = new ModalBuilder()
        .setCustomId('myModal2') // Kullanıcı limiti için modal
        .setTitle('Kullanıcı Limiti');
    ...
    const channelNameInput = new TextInputBuilder()
        ...
        .setRequired(true) // Gerekli hale getirildi
        .setStyle(TextInputStyle.Short);

    // Kullanıcı limiti için alan
    ...
    client.on('interactionCreate', async (interaction) => {
        ...
        // Modal gönderildiğinde etkileşim
        if (interaction.isModalSubmit()) {
            if (interaction.customId === 'myModal') {
                const channelName = interaction.fields.getTextInputValue('hobiler');
                const channel = interaction.guild.channels.cache.get(interaction.channel.id);

                // Kanal adı sayılardan mı oluşuyor diye kontrol et
                if (/^\d+$/.test(channelName)) {
                    // Eğer kanal adı sadece sayılardan oluşuyorsa hata mesajı gönder
                    await interaction.reply({
                        content: 'Kanal adı sadece sayı olamaz, lütfen geçerli bir isim girin.',
                        ephemeral: true // Mesaj sadece kullanıcıya gösterilir
                    });
                } else {
                    if (channel) {
                ...
                if (isNaN(userLimitInput)) {
                    await interaction.reply({
                        ...
                        ephemeral: true  // Yanıtı sadece kullanıcıya göster
                    });
                } else {
                    const userLimit = parseInt(userLimitInput);  // String değeri sayıya çevir
                    const channel = interaction.guild.channels.cache.get(interaction.channel.id);
                    if (channel) {
                        // Kullanıcı limiti 99'dan fazla mı?
                        if (userLimit > 99) {
                            await interaction.reply({
                                content: 'Uyarı: Kullanıcı limiti 99\'dan fazla olamaz. Lütfen geçerli bir sayı girin.',
                                ephemeral: true // Uyarı mesajı sadece kullanıcıya gösterilecek
                            });
                        } else {
